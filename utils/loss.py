...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model
        ...
        :params p: 模型输出的预测框，由模型构建中的三个检测头Detect返回的三层输出
        ...
        # [tensor([80, 80, 80, 80]), tensor([40, 40, 40, 40]), tensor([20, 20, 20, 20])]
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Regression 只计算所有正样本的目标框回归损失
                grid = torch.stack([gi, gj], dim=1)
                # pxy = [-0.5 + cx, 1.5 + cx]    pwh = [0, 4pw]   这个区域内都是正样本
                pxy = ps[:, :2].sigmoid() * 2. - 0.5
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            # 取前十之和并取整(比如：0.8+0.6+0.7+...+0.5 = 6.7 ≈ 7)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n_aux:
                ...
                lbox += 0.25 * (1.0 - iou_aux).mean()  # iou loss Aux辅助头检测占比小(Coarse 粗略)
